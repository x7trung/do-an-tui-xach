...
const ProductItem = ({ data, noLike }) => {
    ...
    const handleAddToCart = () => {
        ...
        if (data.detail[activeColor].quantity < 1) {
            ...
            addToCart(auth.id, {
                product_id: data._id, product_color: [...new Set(data.detail.map(i => i.color))][activeColor], product_quantity: 1, product_price: data.price, product_image: data.image, product_name: data.name, product_code: data.product_code
            })
        setShow(!show)
    }
